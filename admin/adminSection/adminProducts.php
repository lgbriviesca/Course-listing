...
$courseAuthor = (isset($_POST["courseAuthor"])) ? $_POST["courseAuthor"] : "";
...
$selectOption = (isset($_POST["taskOption"])) ? $_POST["taskOption"] : 5;
...
switch ($action) {
    case "Add":
        $addSentence = $conection->prepare("INSERT INTO cursos (title, picture, author, details) VALUES (:title, :picture, :author, :details);");
        $addSentence->bindParam(":title", $courseName);
        $addSentence->bindParam(":author", $courseAuthor);
        $addSentence->bindParam(":details", $courseDetails);
        $date = new DateTime();
        ...
        break;
    case "Modify":
        $updateSentence = $conection->prepare("UPDATE cursos SET title = :title, author = :author, details = :details WHERE id = :id");
        $updateSentence->bindParam(":title", $courseName);
        $updateSentence->bindParam(":author", $courseAuthor);
        $updateSentence->bindParam(":details", $courseDetails);
    ...
    case "Select":
        ...
        $courseAuthor = $courseShow["author"];
        $courseDetails = $courseShow["details"];
        ...
        <div class="card-body">
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input required type="text" value="<?php echo $courseAuthor ?>" class="form-control" name="courseAuthor" id="courseAuthor" placeholder="course author">
                </div>
                <div class="form-group">
                    <label for="courseDetails">Details:</label>
                    <textarea required rows="5" maxlength="990" class="form-control" name="courseDetails" id="courseDetails" placeholder="course content"><?php echo $courseDetails ?></textarea>
                </div>
...
<div class="col-md-7 productsWithAction">
    ...
    <div class="">
        <form action="" method="POST">
            <select name="taskOption">
                <option disabled selected>Items to show</option>
    ...
    <ul class="pagination paginationDown">
        <?php if ($page > 1) { ?>
            <li class="page-link">
                <a href="./adminProducts.php?pagina=<?php echo $page - 1 ?>">
        ...
        <?php for ($x = 1; $x <= $pages; $x++) { ?>
            <li class="<?php if ($x == $page) echo "active" ?> page-link">
                <a href="./adminProducts.php?pagina=<?php echo $x ?>">
                    <?php echo $x ?></a>
            </li>
        <?php } ?>
        <?php if ($page < $pages) { ?>
